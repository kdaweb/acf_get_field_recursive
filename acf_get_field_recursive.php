 ...
 *   v1.0.3: bug fixes
 *   v1.0.4: bug fixes
 *   v1.0.5: inclusion of test content; now explicitly
 *           requires ACF at activation
 *   v1.0.6: phpcbf (with wpcs) reformatting;
 *
 * PHP Version 5.2.4
 * @category   CustomFields
 * @package    ACFGetFieldRecursive
 ...
 * Description:       recursive ACF shortcode that searches the post's ancestors
 * Version:           1.0.6
 ...
 * Text Domain:       ACFGetFieldRecursive
...
function acf_recursive($attributes) {
  ...
  $return_value = get_field_recursive(
    $atts['field'],
    $atts['post_id'],
    $atts['format_value']
  );
 ...
 * @since  1.0.0
 */
function get_field_recursive($field, $post_id = NULL, $format_value = NULL) {
  ...
  if ($return_value) {
    ...
    if ($ancestors) {
      ...
      $parent_value = get_field_recursive($field, $parent_id, $format_value);
      return $parent_value;
    } else {
      // if value is not found and there are no more ancestors,
      // return false
      return FALSE;
    } // end if()
  } // end if()
...
function verify_acf_activated() {
  ...
  if ((! is_plugin_active('advanced-custom-fields/acf.php'))
    ...
    $message     = <<<EOM
This plugin requires Advanced Custom Fields to be installed and active.<br />
<a href='$return_link'>&laquo; Return to Plugins</a>
EOM;

    // Stop activation redirect and show error
    // see: https://codex.wordpress.org/Function_Reference/wp_die
    wp_die($message);
  } // end if()

    return TRUE;
} //end verify_acf_activated()
